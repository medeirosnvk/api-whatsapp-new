...
class StateMachine {
      ...
      where
        ...
        `;
            ...
            if (dbResponse && dbResponse.length) {
                ...
                console.log(
                    `Nenhum credor encontrado para o número ${phoneNumber}.`
                );
                return null;
            }
        } catch (error) {
    ...
    async _handleMenuState(origin, phoneNumber, response) {
        ...
        switch (initialStateResponse) {
            ...
            case "3":
                try {
                    await this._handleBoletoState(
                        origin,
                        phoneNumber,
                        response
                    ); // Passando o phoneNumber e response como argumentos
                } catch (error) {
    ...
    async _handleCredorState(origin, phoneNumber, response) {
        try {
            ...
            if (Array.isArray(credorInfo) && credorInfo.length > 0) {
                ...
                if (selectedCreditor) {
                    this._setDataCredorSelecionado(
                        phoneNumber,
                        selectedCreditor
                    );
    ...
    async _handleOfertaState(origin, phoneNumber, response) {
        try {
            if (response && response.body.trim().match(/^\d+$/)) {
                const selectedOptionParcelamento = parseInt(
                    response.body.trim()
                );
                ...
                const credorOfertas = await requests.getCredorOfertas(
                    iddevedor
                );
                ...
                if (
                    ...
                    const ultimaDataFormat = ultimaData
                        .toISOString()
                        .slice(0, 10);
                    ...
                    const formattedDate = newDataBase
                        .toString()
                        .substring(0, 10);
                    ...
                    const responseDividasCredores =
                        await requests.getCredorDividas(
                            iddevedor,
                            ultimaDataFormat
                        );
      ...
      `;
                        ...
                        const promise = await requests.postDadosPromessa(
                            dataPromessa
                        );
                    ...
                    const responseRecibo = await requests.postDadosRecibo(
                        parsedData3
                    );
                    ...
                    const responseIdBoleto = await requests.getIdBoleto(
                        idacordo
                    );
                    ...
                    const updateValoresBoleto =
                        await requests.postAtualizarValores({
                            idboleto,
                            banco,
                            convenio,
                        });
                    ...
                    if (
                        ...
                        console.error(
                            "Erro ao atualizar valores de nossoNum e numDoc: ",
                            {
                                updateValoresBoleto,
                            }
                        );
                    ...
                    const responseBoletoContent =
                        await requests.getImagemBoleto(parsedData4);

                    const parsedData5 = utils.parseDadosImagemQrCode({
                        idboleto,
                    });

                    const responseQrcodeContent =
                        await requests.getImagemQrCode(parsedData5);
    ...
    async _handleAcordoState(origin, phoneNumber, response) {
        try {
            ...
            if (!acordosFirmados || acordosFirmados.length === 0) {
                ...
                const formatAcordos =
                    utils.formatCredorAcordos(acordosFirmados);
        ...
        try {
            ...
            if (!acordosFirmados || acordosFirmados.length === 0) {
                ...
                for (const acordo of acordosFirmados) {
                    ...
                    try {
                        const responseBoletoPix =
                            await requests.getDataBoletoPix(iddevedor);
...
const createSession = async (sessionName) => {
    ...
    try {
        ...
        client.on("authenticated", (data) => {
            ...
            console.log(
                `Conexão bem-sucedida na sessão ${client.sessionName}!`
            );
        ...
        client.on("message", async (message) => {
            try {
                ...
                console.log(
                    `Sessão ${sessionName} recebeu a mensagem: ${
                        message.body
                    } de ${message.from} no horário ${new Date()}`
                );
                ...
                try {
                    const credorExistsFromDB =
                        await stateMachine._getCredorFromDB(fromPhoneNumber);
                    ...
                    const statusAtendimento =
                        await requests.getStatusAtendimento(fromPhoneNumber);
                    if (
                        statusAtendimento[0] &&
                        statusAtendimento[0].bot_idstatus
                    ) {
                    ...
                    if (
                        ...
                        console.log(
                            "Usuário em atendimento automático -",
                            bot_idstatus
                        );
                    }

                    const ticketStatus =
                        await requests.getTicketStatusByPhoneNumber(
                            fromPhoneNumber
                        );
                    ...
                    if (ticketStatus && ticketStatus.length > 0) {
                        ...
                        const insertNovoTicket =
                            await requests.getInserirNovoTicket(
                                fromPhoneNumber
                            );
                        if (insertNovoTicket && insertNovoTicket.insertId) {
                            ...
                            console.log(
                                "Erro ao criar novo número de Ticket no banco."
                            );
                            return;
                        }
                    }
...
const saveClientData = (client) => {
    ...
    try {
        ...
        console.log(
            `Dados da sessão ${client.sessionName} salvos em ${filePath}`
        );
    } catch (error) {
...
const disconnectSession = async (sessionName) => {
    ...
    if (client) {
        try {
            ...
            console.log(
                `Sessão ${sessionName} removida da memória com sucesso.`
            );
            ...
            console.error(
                `Erro ao desconectar a sessão ${sessionName}:`,
                error
            );
            throw error;
        }
...
app.delete("/instance/clearUnusedSessions", (req, res) => {
    try {
        ...
        res.status(200).json(
            "Sessões não utilizadas foram removidas com sucesso."
        );
    } catch (error) {
...
app.get("/instance/fetchInstances", (req, res) => {
    ...
    if (fs.existsSync(clientDataPath)) {
        ...
        console.log(
            "Arquivo clientData.json não encontrado em:",
            clientDataPath
        );
...
app.post("/sendMessage", async (req, res) => {
    ...
    try {
        ...
        const mediaData = Buffer.from(response.data, "binary").toString(
            "base64"
        );
...
app.post("/message/sendMedia/:instanceName", async (req, res) => {
    ...
    try {
        ...
        if (error.message.includes("disconnected")) {
            ...
            console.error(
                `Erro desconhecido ao enviar mensagem: ${error.message}`
            );
        }
